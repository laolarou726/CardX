...
struct RandomMusicTimelineProvider: IntentTimelineProvider {
    ...
    func placeholder(in context: Context) -> RandomMusicEntry {
        RandomMusicEntry(date: Date(), configuration: ConfigurationIntent())
    }

    func getSnapshot(for configuration: ConfigurationIntent, in context: Context, completion: @escaping (RandomMusicEntry) -> ()) {
        let entry = RandomMusicEntry(date: Date(), configuration: configuration)
        completion(entry)
    }

    func getTimeline(for configuration: ConfigurationIntent, in context: Context, completion: @escaping (Timeline<RandomMusicEntry>) -> ()) {
        AF.request("https://netease-cloud-music-api-sigma-taupe.vercel.app/playlist/track/all?id=\(playlistId)&limit=\(limit)&offset=1&cookie=\(cookie.addingPercentEncoding(withAllowedCharacters: .urlHostAllowed)!)").responseData{
            response in
